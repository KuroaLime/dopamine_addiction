...
#include "Game/InGame/MainPlayerState.h"          // 추가
...
{
    if (CachedPlayerState.IsValid()) return; // 이미 등록됨

    APlayerController* PC = GetOwningPlayer();
    if (!PC) return;

    AMainPlayerState* PS = PC->GetPlayerState<AMainPlayerState>();
    if (!PS) return; // 아직 리플리케이션 안 됨 → Tick에서 재시도
    ...
    bNeedPlayerStateBind = false; // 성공 → 재시도 중단
}
